document.addEventListener("DOMContentLoaded", function () {
  ...
  var starterCourses = [
    { dept: "ITCS", num: "3112", name: "Design and Implementation of Object Oriented Systems", reason: "I want to learn more about object oriented programming and its implementations." },
    { dept: "ITIS", num: "3135", name: "Front-End Web Application Development", reason: "I am interested in being a front-end developer." },
    { dept: "ITSC", num: "3146", name: "Intro to Operating Systems and Networks", reason: "To see more of the basis between systems and networks." },
    { dept: "STAT", num: "2122", name: "Intro to Prop and Stat", reason: "Required but I have always been interested in statistics." },
    { dept: "ITIS", num: "3130", name: "Introduction to Human Centered Computing", reason: "It seemed interesting with more theory and implementation." }
  ];
  ...
  function addCourse(course) {
    ...
    block.innerHTML =
      '<label>Department:<input type="text" class="courseDept" value="' + (course.dept || "") + '" required></label>' +
      '<label>Number:<input type="text" class="courseNum" value="' + (course.num || "") + '" required></label>' +
      '<label>Course Name:<input type="text" class="courseName" value="' + (course.name || "") + '" required></label>' +
      '<label>Reason:<input type="text" class="courseReason" value="' + (course.reason || "") + '" required></label>' +
      '<button type="button" class="deleteCourse">Delete</button><hr>';
  ...
  function buildResult() {
    ...
    resultDiv.innerHTML =
      '<h2>Introduction</h2>' +
      '<figure><img src="' + finalImage + '" width="200" height="200">' +
      '<figcaption><em>' + caption + '</em></figcaption></figure>' +
      '<ul>' +
  ...
  var i;
  for (i = 0; i < starterCourses.length; i++) addCourse(starterCourses[i]);
  ...
  clearBtn.addEventListener("click", function () {
    ...
    for (i = 0; i < fields.length; i++) {
      if (fields[i].type !== "file") fields[i].value = "";
    }
  });
